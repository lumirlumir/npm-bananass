...
function isInExternalModuleFolder(name, context) {
  ...
  return folders.some(folder => {
    if (nodeIsAbsolute(folder)) {
      try {
        statSync(nodeResolve(folder, base));
        return true;
      } catch {
        return false;
      }
...
function typeTest(name, context, path) {
  ...
  if (path && isExternalLookingName(name) && isInExternalModuleFolder(name, context)) {
    return 'external';
  }
...
export function isExternalModule(name, path, context) {
  ...
  return (
    (isModule(name) || isScoped(name)) && typeTest(name, context, path) === 'external'
  );
}
